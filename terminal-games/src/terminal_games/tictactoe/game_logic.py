...
def get_best_move(board: Board) -> int:
    ...
    result = minimax(board[:], AI_PLAYER)  # Pass a copy
    return result["index"]
...
def make_move(state: GameState, index: int) -> GameState:
    ...
    # Human move
    new_board[index] = HUMAN_PLAYER

    # Check human win
    human_win = check_win(new_board, HUMAN_PLAYER)
    if human_win is not None:
        new_state.is_game_over = True
        new_state.winner = "You Win!"
        new_state.winning_cells = WIN_COMBOS[human_win]
        return new_state

    # Check tie
    if check_tie(new_board):
        new_state.is_game_over = True
        new_state.winner = "Tie Game!"
        return new_state

    # AI move
    ai_move = get_best_move(new_board)
    new_board[ai_move] = AI_PLAYER

    # Check AI win
    ai_win = check_win(new_board, AI_PLAYER)
    if ai_win is not None:
        new_state.is_game_over = True
        new_state.winner = "You Lose!"
        new_state.winning_cells = WIN_COMBOS[ai_win]
        return new_state

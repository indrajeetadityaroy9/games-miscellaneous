...
class GameState:
    ...
    Note: chess.Board is inherently mutable (external library).
    We use copy() before mutations to maintain immutability semantics.
    """
    board: chess.Board
...
def push_move(state: GameState, move: chess.Move) -> GameState:
    """Push a move and return new state with updated board."""

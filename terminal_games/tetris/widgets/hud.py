"""HUD widget for displaying Tetris game status."""
...
class HUD(Widget):
    """Heads-up display showing score, level, lines, and status."""
    ...
    def compose(self):
        ...
        yield Static(id="lines-display", classes="hud-stat")
    ...
    def _update_lines_display(self) -> None:
        try:
            display = self.query_one("#lines-display", Static)
            display.update(f"[dim]LNS[/] {self.lines}")
        except Exception:
            pass
    ...
    def _update_score_display(self) -> None:
        try:
            ...
            display.update(f"[dim]SCR[/] {self.score:06d}")
        except Exception:
            pass
    ...
    def _update_high_score_display(self) -> None:
        try:
            ...
            display.update(f"[dim]HI[/] {self.high_score:06d}")
        except Exception:
            pass
